...
import {
	useDistrictDeleteShippingMutation,
	useDistrictUpdateShippingMutation
} from '../../../../api/districts/useDistricts';
...
function DistrictAvatar({ district, size = 30 }) {
...
function EditDistrictShippingRouteDrawer({ route, originDistrict, originCountry, districts, onClose }) {
	const [confirmingDelete, setConfirmingDelete] = useState(false);
	const updateRoute = useDistrictUpdateShippingMutation();
	const deleteRoute = useDistrictDeleteShippingMutation();
	...
	const destinationDistrict = resolveDistrict(route?._dest || route?.districtToShipTo, districts);
	...
	function getDestinationId() {
		if (destinationDistrict?._id) return destinationDistrict._id;
		if (typeof route?.districtToShipTo === 'string') return route.districtToShipTo;
		return null;
	}
	...
	function onSubmit(values) {
		updateRoute.mutate({
			...
			districtToShipTo: getDestinationId(),
	...
	return (
		<Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
			...
			>
				...
				>
					...
					>
						...
						<DistrictAvatar district={originDistrictData} />
						<FuseSvgIcon
							size={16}
						...
						<DistrictAvatar district={destinationDistrict} />
						<Box>
							<Typography
								variant="caption"
							...
							>
								{originDistrictData?.name} → {destinationDistrict?.name || 'Destination'}
							</Typography>
						</Box>
					</Stack>
					<Typography
					...
					>
						Update freight rates and transit times for this district-to-district route
					</Typography>
				</motion.div>
			</Box>
			...
			>
				...
				>
					Destination district cannot be changed. Delete and recreate to change the destination.
				</Typography>
			</Box>
				...
				>
					...
					{confirmingDelete ? (
						...
						>
							...
							>
								Remove route to {destinationDistrict?.name || 'this district'}?
							</Typography>
							<Button
								size="small"
...
export default EditDistrictShippingRouteDrawer;
