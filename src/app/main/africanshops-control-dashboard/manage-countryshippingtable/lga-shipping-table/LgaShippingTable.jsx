...
function LgaAvatar({ lga, size = 36 }) {
...
function EmptyState({ selectedCountry, selectedState, originLga }) {
	let title;
	let message;
	...
	if (!selectedCountry) {
		...
		title = 'Choose an origin LGA';
		message = `Select an LGA within ${selectedState.name} to view and manage its outbound shipping routes.`;
	} else {
		title = 'No routes configured yet';
		message = `${originLga?.name || 'This LGA'} has no outbound LGA routes. Click "Add Route" to define freight rates to destination LGAs.`;
	}
...
function LgaShippingTable({ originLga, selectedState, selectedCountry, lgas, onEditRoute }) {
	const [page, setPage] = useState(0);
	const [rowsPerPage, setRowsPerPage] = useState(10);

	// The full-record response may nest under `.lga` or expose the array directly
	const routes = useMemo(
		() => originLga?.lga?.shippingTable || originLga?.shippingTable || [],
		[originLga]
	);

	const routesWithDest = useMemo(
		() => routes.map((r) => ({ ...r, _dest: resolveLga(r.lgaToShipTo, lgas) })),
		[routes, lgas]
	);
	...
	const lgaName = originLga?.lga?.name || originLga?.name;
								...
								>
									<TableCell sx={{ py: 2 }}>
										<Stack
											...
											spacing={1.5}
										>
											<LgaAvatar lga={route._dest} />
											<Box>
												<Typography
													variant="body2"
													fontWeight={700}
												>
													{route._dest?.name || route.lgaToShipToName || 'Unknown'}
												</Typography>
												<Typography
													variant="caption"
													color="text.secondary"
...
export default LgaShippingTable;
