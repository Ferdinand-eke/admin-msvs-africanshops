...
import { getOperationalLgaByStateId, getStateByCountryId } from 'src/app/api/apiRoutes';
...
function BasicInfoTab() {
	...
	useEffect(() => {
		...
		if (getValues()?.businezstate?.length > 0) {
			console.log('Getting L>G<As....');
			getLgasFromState(getValues()?.businezstate);
		}
	}, [getValues()?.businesscountry, getValues()?.businezstate, getValues()?.businezLga, getValues()?.makeWarehouse]);
	...
	async function getLgasFromState(sid) {
		...
		console.log('L.G.As', responseData?.data?.lgas);
		...
		<div>
					...
					<Controller
						...
						render={({ field: { onChange, value } }) => (
							<Select
								...
								//  {...(error && {error: true, helperText: error})}
							>
								<MenuItem value="">Select a county/l.g.a</MenuItem>
								{blgas &&
									blgas?.map((option, id) => (
										<MenuItem
											key={option.id}
			...
			<Controller
				...
				render={({ field: { onChange, value } }) => (
					...
					>
						<MenuItem value="">Select a state</MenuItem>
						{marketCatData?.data?.allMarketCats &&
							marketCatData?.data?.allMarketCats?.map((option, id) => (
								<MenuItem
									key={option.id}
									value={option.id}
								>
			...
			<Typography style={{ fontSize: '12px', fontWeight: '800' }}>
				Is this market a warehouse location?
			</Typography>
			<Controller
				name="makeWarehouse"
				control={control}
				...
				render={({ field: { onChange, value } }) => (
					...
					>
						...
						<MenuItem value={false}>Not Warehouse</MenuItem>
