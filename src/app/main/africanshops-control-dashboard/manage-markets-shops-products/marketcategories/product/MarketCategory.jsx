...
function MarketCategory() {
	...
	const {
		...
		isLoading,
		isError
	} = useSingleMarketCategory(productId, {
		...
		return (
			...
			>
				<Typography
					...
					variant="h5"
				>
					There is no such market category!
				</Typography>
				<Button
					className="mt-24"
					component={Link}
	 ...
	 */
	if (_.isEmpty(form) || (category?.data && routeParams.productId !== category?.data._id && routeParams.productId !== 'new')) {
		return <FuseLoading />;
	}
