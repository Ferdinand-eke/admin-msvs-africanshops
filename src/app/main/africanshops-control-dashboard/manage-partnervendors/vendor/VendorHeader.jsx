...
function VendorHeader() {
	...
	function handleCreateProduct() {
		createProduct(getValues())
			.unwrap()
			.then((data) => {
				navigate(`/partnervendors/listpartners/${data.id}`);
			});
	}

	function handleRemoveProduct() {
		removeProduct(productId);
		navigate('/partnervendors/listpartners');
	}
			...
			<div className="flex flex-col items-start space-y-8 sm:space-y-0 w-full sm:max-w-full min-w-0">
				...
				>
					<Typography
						...
						component={Link}
						role="button"
						to="/partnervendors/listpartners"
						color="inherit"
					>
						<FuseSvgIcon size={20}>
							{theme.direction === 'ltr'
						...
						<span className="flex mx-4 font-medium">Partner Vendor</span>
					</Typography>
				</motion.div>
