...
import { getLgaByStateId, getMarketsByLgaId, getMarketsByStateId, getStateByCountryId } from 'src/app/api/apiRoutes';
...
function BasicInfoTab({ vendorId }) {
	...
	return (
		<div>
			...
			{getValues()?.businezState?.length > 0 && (
				<>
					<Typography style={{ fontSize: '12px', fontWeight: '800' }}>
						Choose your shop state location
					</Typography>
					<Controller
						name="businezLga"
						control={control}
			...
			<Typography style={{ fontSize: '12px', fontWeight: '800' }}>Shop Plan to subscribe to?</Typography>
			<Controller
				name="shopplan"
				control={control}
				defaultValue={[]}
